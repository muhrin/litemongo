...
class _Parser:
    ...
    def _parse_basic_expression(self, expression):
        if isinstance(expression, list):
            return list(map(self._parse_basic_expression, expression))
    ...
    def _handle_object_operator(self, operator, values):
        if operator == '$mergeObjects':
            # to_merge = list(map(self.parse, values))
            to_merge = []
            for entry in values:
                try:
                    to_merge.append(self.parse(entry))
                except KeyError:
                    pass
            return _merge_objects_operation(to_merge)

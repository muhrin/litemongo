...
class _Parser(object):
    ...
    def parse(self, expression):
        ...
        if len(expression) > 1 and any(key.startswith("$") for key in expression):
            raise OperationFailure(
                ...
                "the name of the expression. Found %d fields in %s" % (len(expression), expression)
            )
    ...
    def _handle_arithmetic_operator(self, operator, values):
        if operator in unary_arithmetic_operators:
            ...
            if not isinstance(number, numbers.Number):
                raise OperationFailure(
                    "Parameter to %s must evaluate to a number, got '%s'" % (operator, type(number))
                )
        ...
        if operator == "$regexMatch":
            ...
            for option in values.get("options", ""):
                if option not in "imxs":
                    raise OperationFailure("$regexMatch invalid flag in regex options: %s" % option)
    ...
    def _handle_control_flow_operator(self, operator, values):
        if operator == "$switch":
            if not isinstance(values, dict):
                raise OperationFailure(
                    "$switch requires an object as an argument, " "found: %s" % type(values)
                )

            branches = values.get("branches", [])
            if not isinstance(branches, (list, tuple)):
                raise OperationFailure(
                    "$switch expected an array for 'branches', " "found: %s" % type(branches)
                )
            if not branches:
            ...
            for branch in branches:
                if not isinstance(branch, dict):
                    raise OperationFailure(
                        "$switch expected each branch to be an object, " "found: %s" % type(branch)
                    )
...
def _handle_graph_lookup_stage(in_collection, database, options):
    ...
    for operator in ("as", "connectFromField", "connectToField", "from"):
        if operator not in options:
            raise OperationFailure("Must specify '%s' field for a $graphLookup" % operator)
        if not isinstance(options[operator], str):
            raise OperationFailure("Argument '%s' to $graphLookup must be string" % operator)
...
def _handle_bucket_stage(in_collection, unused_database, options):
    ...
    if unknown_options:
        raise OperationFailure("Unrecognized option to $bucket: %s." % unknown_options.pop())
    ...
    if not isinstance(boundaries, list):
        raise OperationFailure(
            "The $bucket 'boundaries' field must be an array, but found type: %s" % type(boundaries)
        )
    if len(boundaries) < 2:
        raise OperationFailure(
